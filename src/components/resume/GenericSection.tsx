...
const GenericSection = ({ title, items }: GenericSectionProps) => {
  return (
      ...
      {items.map((item, index) => (
        <div key={index} className="mb-4 print:mb-3">
          ...
          {item.company && item.date && (
            <p className="text-sm text-slate-600 mb-2 print:text-black">
              <strong>{item.company}</strong> | <em>{item.date}</em>
            </p>
          )}
